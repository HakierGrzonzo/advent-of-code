...
def try_do_step(pre_steps: int):
    ...
    for move_number, (falling_x, falling_y) in zip(range(pre_steps), points_iter):
        node = grid[falling_x][falling_y]
        node.destroy()
        grid[falling_x][falling_y] = None
        board[falling_x][falling_y] = "#"

    def expand_points_iter():
        yield from points_iter
        while True:
            yield None

    move_number = 0
    for falling in expand_points_iter():
        current_x, current_y = current_pos
        current_node = grid[current_x][current_y]
        board[current_x][current_y] = "O"
        ...
        for row in grid.values():
            for n in row.values():
                if n is not None:
                    nodes.append(n)
        if current_node not in nodes or goal_node not in nodes:
            return False
